...
(define (plist->alist d)
  (match pl
    [(list 'dict entries ...) (map plist->alist entries)]
    [(list 'assoc-pair k v) (cons (string->symbol k) (plist->alist v))]
    [(list 'integer n) n]
    [(list 'real n) n]
    [(? string? s) s]
    [(list 'false) #f]
    [(list 'true) #t]
    [(list 'array elts ...) (map plist->alist elts)]))
